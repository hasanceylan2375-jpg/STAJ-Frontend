...
export class MusteriCard {
  ...
  get initials(): string { const first=String(this.customer?.ad??'').trim().charAt(0); const last=String(this.customer?.soyad??'').trim().charAt(0); return (first+last||'M').toUpperCase(); }
  get fullName(): string { return `${this.customer?.ad??''} ${this.customer?.soyad??''}`.trim() || 'Müşteri'; }
  get statusLabel(): string { return this.status==='active'?'Aktif':this.status==='passive'?'Pasif':'Potansiyel'; }
