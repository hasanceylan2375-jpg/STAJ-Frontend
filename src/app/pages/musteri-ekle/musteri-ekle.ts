...
export class MusteriEkle implements OnInit {
  ...
  kaydet(): void { this.yukleniyor = true; if (this.secilenDosya) { this.imageService.upload(this.secilenDosya).subscribe({ next: r => { this.profilFotoUrl = r.url; this.musteriTalebiniGonder(); }, error: e => { this.yukleniyor = false; this.toastService.error(e?.message || 'Fotoğraf yüklenemedi!'); } }); return; } this.musteriTalebiniGonder(); }
  private musteriTalebiniGonder(): void {
    ...
    if (this.guncellenenId !== null) { this.musteriService.musteriGuncelle(this.guncellenenId, { ...musteri, id: this.guncellenenId }).subscribe({ next: (r: any) => { this.yukleniyor = false; this.toastService.success(r?.message || 'Müşteri başarıyla güncellendi!'); this.formuTemizle(); }, error: (e: any) => { this.yukleniyor = false; this.toastService.error(e?.error?.message || 'Güncelleme başarısız!'); } }); return; }
    this.workflowService.start(musteri).subscribe({ next: () => { this.yukleniyor = false; this.toastService.success('Müşteri ekleme talebi müdür onayına gönderildi.'); this.formuTemizle(); this.router.navigate(['/workflow-taleplerim']); }, error: (e: any) => { this.yukleniyor = false; this.toastService.error(e?.error?.message || 'Müşteri ekleme talebi oluşturulamadı!'); } });
  }
